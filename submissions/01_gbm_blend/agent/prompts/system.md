...
1. **Train and predict in one step** using the bundled skill (do not write your own modeling code
   ...
   This trains a cross-validated LightGBM+XGBoost+CatBoost blend and writes `submission.csv` to the
   working directory. It prints a JSON summary with the cross-validated AUC.
